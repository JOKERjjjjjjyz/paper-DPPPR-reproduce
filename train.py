...
def Ktop_single(vector_origin,vector_propagate,M,N,k,test,adversary):
    #input:vector_origin是csr.getrow,貌似还是csr. vector_propagate是ndarray：（xx,）
    #output:单测试用户id的recall, precision, F1,ndcg
    count = 0
    test_matrix = np.zeros((M,k))
    r_matrix = np.zeros((M,k))
    #把origin转成了ndarray为vector_origin_v:(xx,)，内部全是用ndarray进行运算
    vector_origin_v = vector_origin.toarray().reshape(-1)
    #print(vector_origin_v.shape,vector_propagate.shape) (xx,)(xx,)
    mask = (vector_origin_v == 0).astype(int)
    #print(mask.shape) (xx,)
    vector = vector_propagate * mask
    ...
    print(f"adversary{adversary}_recall:{recall}, adversary{adversary}_precision:{precision}, adversary{adversary}_F1:{F1}, adversary{ndcg_value}_recall:{ndcg_value}")
